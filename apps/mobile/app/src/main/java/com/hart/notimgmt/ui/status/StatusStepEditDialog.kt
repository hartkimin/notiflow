...
private const val MAX_STATUS_NAME_LENGTH = 20

@Composable
fun StatusStepEditDialog(
    step: StatusStepEntity? = null,
    ...
    var selectedColor by remember {
        mutableIntStateOf(step?.color ?: predefinedColors.first().toArgb())
    }
    ...
    val nameError by remember(name) {
        derivedStateOf {
            ...
            when {
                ...
                trimmed.length > MAX_STATUS_NAME_LENGTH -> "${MAX_STATUS_NAME_LENGTH}자 이내로 입력해주세요"
                existingNames.any { it.equals(trimmed, ignoreCase = true) && it != step?.name } ->
                    "이미 존재하는 상태 이름입니다"
                else -> null
            }
        }
    ...
    AlertDialog(
        ...
        title = {
            Text(
                text = if (step == null) "상태 단계 추가" else "상태 단계 수정",
        ...
        text = {
            Column {
                OutlinedTextField(
                    value = name,
                    onValueChange = { if (it.length <= MAX_STATUS_NAME_LENGTH + 5) name = it },
                    label = { Text("단계 이름") },
                    singleLine = true,
