package com.hart.notimgmt.data.notiroute
...
@Serializable
private data class NotiRouteMessageBody(
...
@Singleton
class NotiRouteApiClient @Inject constructor(
    private val httpClient: HttpClient,
    private val appPreferences: AppPreferences
) {
    companion object {
        private const val TAG = "NotiRouteApiClient"
    }
    ...
    suspend fun sendMessage(message: CapturedMessageEntity) {
        ...
        val jsonBody = json.encodeToString(NotiRouteMessageBody.serializer(), body)
        Log.d(TAG, "Sending to NotiRoute: $jsonBody")
        ...
        Log.d(TAG, "NotiRoute response: ${response.status} - ${response.bodyAsText()}")
    }

    /**
     * Test connection to the NotiRoute API Gateway.
    ...
    suspend fun testConnection(): Pair<Boolean, String> {
        ...
        return try {
            val response = httpClient.post("$apiUrl/api/v1/messages") {
                contentType(ContentType.Application.Json)
                header(HttpHeaders.Authorization, "Bearer $apiKey")
                setBody("""{"source_app":"other","sender":"test","content":"NotiRoute 연결 테스트","received_at":"${epochToIso8601(System.currentTimeMillis())}","device_id":"${Build.MODEL}"}""")
            }
