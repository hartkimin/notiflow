...
웹 성능 최적화 — 전체 페이지 로딩 속도 개선 (번들 축소, 데이터 페칭 최적화, 컴포넌트 메모이제이션).

### Web Dashboard (`apps/web`)

#### Dynamic Import (번들 축소)
- **5개 무거운 컴포넌트 지연 로드**: TrendChart, OrderTable, OrderCalendar, MfdsSearchPanel, MessageCalendar
- 각 컴포넌트에 Skeleton 로딩 상태 추가
- 초기 JS 번들에서 Recharts (~100KB+) 및 대형 테이블 컴포넌트 분리

#### Lazy 드롭다운 검색 (데이터 페칭 최적화)
- **SearchableCombobox**: debounce(300ms) 서버 액션 기반 검색 콤보박스 신규 생성
- 주문 페이지에서 **1000개 병원 + 1000개 공급자 프리로드 쿼리 제거**
- 병원/공급자 드롭다운: 타이핑 시 상위 20개 결과만 서버에서 검색

#### 탭 데이터 분리
- 주문/알림 페이지: 캘린더 데이터를 **탭 클릭 시에만** 로드
- 주문 페이지 쿼리: 6개 → 3개, 알림 페이지 쿼리: 5개 → 3개

#### Realtime 스크롤 보존
- DB 변경 시 `router.refresh()` 후 **스크롤 위치 자동 복원**

#### 컴포넌트 메모이제이션
- `OrderGroupRow`: `React.memo()` — 관련 없는 상태 변경 시 리렌더 방지
- `TrendChart`: `memo()` + `useMemo()` — 데이터 변환 캐싱

---

## v0.11.0 — 2026-02-27

수신메시지 기능 복원 — `captured_messages` 테이블 기반으로 메시지 목록/캘린더/예측 기능 복구.

### Web Dashboard (`apps/web`)

#### 수신메시지 인박스 복원
- **2패널 인박스**: 목록 + 상세 패널 (기존 3패널에서 주문 패널 제거)
- **메시지 목록**: 발신자, 수신시간, 내용 미리보기, 출처 뱃지 표시
- **메시지 상세**: 내용 보기/편집, 코멘트, 핀 고정, 복사, 삭제
- **로컬 상태 관리**: 상태 단계(접수→확인중→처리중→완료), 핀, 코멘트 (localStorage)
- **일괄 삭제**: 체크박스 선택 후 일괄 소프트 삭제

#### 목록/캘린더 탭 전환
- **목록 탭**: 날짜 범위, 출처 필터, 페이지네이션
- **캘린더 탭**: 일/주/월 뷰, 메시지+예측 통합 표시

#### 예측(Forecast) 기능
- **예상 주문 등록**: 거래처 + 날짜 + 품목 선택
- **주간 예상 일괄 등록**: 요일 선택으로 일괄 생성
- **캘린더 더블클릭**: 해당 날짜로 예상 등록 다이얼로그 자동 열기

#### 기술 변경점
- `raw_messages` → `captured_messages` 테이블 전환
- 메시지 ID: number → string (모바일 UUID)
- 수신시간: ISO timestamp → epoch ms (bigint)
- 파싱 관련 UI 전부 제외 (AI 파싱, 파싱 실행, 파싱 결과 테이블)
- 경로: `/messages` → `/notifications`

---

## v0.10.0 — 2026-02-27

품목 검색 페이지 UI/UX 전면 개선 — 컬럼 지정 검색, 확장 필터, 레이아웃 최적화, 상세 정보 완전 표시.

### Web Dashboard (`apps/web`)

#### 메인 검색 기능 개선
- **컬럼 선택 드롭다운**: 검색 입력 왼쪽에 Select 추가. 의약품 8개 / 의료기기 7개 컬럼 중 선택 가능
- **"전체" 모드**: 기본값. 품목명 → 업체명 → 코드 순으로 순차 검색 (기존 스마트 감지 대체)
- **탭 연동**: 의약품/의료기기 탭 전환 시 드롭다운 옵션 자동 변경

#### 필터 기능 확장
- **전체 컬럼 필터**: 의약품 21개 / 의료기기 20개 전체 컬럼 필터 가능 (기존 5~6개에서 확대)
- **타입별 연산자**: 텍스트(포함/일치/시작/제외), 날짜(이전/이후/범위), 상태(일치)
- **AND/OR 토글**: 2개 이상 필터 시 AND/OR 조건 선택 (AND=API, OR=클라이언트)

#### 결과 내 검색 위치 재조정
- "결과 내 검색" 필드를 총 건수 옆(좌측)으로 이동하여 검색 흐름 자연스럽게 배치

#### 컬럼 표시 버튼 개명
- "컬럼" → **"표시 항목"**, 드롭다운 제목 "표시할 컬럼" → **"표시할 항목"**

#### 컬럼 너비 더블클릭 auto-fit
- 컬럼 헤더 더블클릭 시 해당 컬럼 내용에 맞게 자동 너비 조정 (100~500px 범위)

#### 아코디언 상세 정보 확장
- **전체 필드 표시**: 의약품 24개 / 의료기기 20개 필드 빠짐없이 표시 (빈 값은 "-")
- **복사 버튼**: 각 필드 값 옆에 클립보드 복사 아이콘 (비HTTPS 환경 fallback 포함)
- **긴 텍스트 줄바꿈**: `whitespace-pre-wrap` 적용으로 말줄임 없이 전체 표시
