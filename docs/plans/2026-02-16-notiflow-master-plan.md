...
| # | 영역 | 과제 | 산출물 |
|---|------|------|--------|
| 1.1 | C | DB Webhook 프로덕션 설정 | Webhook 설정 문서 |
| 1.2 | C | Edge Function 배포 | 4개 함수 프로덕션 배포 |
| 1.3 | D | Vercel 환경변수 설정 | 프로덕션 배포 완료 |
| 1.4 | B | 에러 핸들링 통합 | 모든 Server Action 에러 토스트 |
| 1.5 | A | FCM 토큰 등록 | 푸시 알림 수신 동작 |
| 1.6 | 전체 | E2E 연동 테스트 | 메시지→주문→알림 전체 흐름 검증 |

### Phase 2: 실시간 강화 (2주 ~ 4주)

> 목표: 모든 클라이언트에서 실시간 데이터 반영

| # | 영역 | 과제 | 산출물 |
|---|------|------|--------|
| 2.1 | B | Realtime 구독 전체 적용 | 모든 목록 페이지 실시간 갱신 |
| 2.2 | A | 주문 상태 실시간 추적 | 모바일에서 주문 상태 변경 알림 |
| 2.3 | A | 오프라인 큐 구현 | 네트워크 끊김 시 메시지 큐잉 |
| 2.4 | A | 동기화 충돌 해결 | 충돌 감지 + 해결 UI |

### Phase 3: 비즈니스 기능 확장 (4주 ~ 8주)

> 목표: 실무에서 필요한 핵심 비즈니스 기능 추가

| # | 영역 | 과제 | 산출물 |
|---|------|------|--------|
| 3.1 | B | 주문서 PDF 출력 | PDF 생성 + 다운로드 |
| 3.2 | B | 메시지 수동 파싱 UI | 파싱 실패 메시지 수동 매칭 |
| 3.3 | B | 대시보드 분석 고도화 | 트렌드 차트, 병원별 분석 |
| 3.4 | C | 배송 추적 테이블 + API | deliveries 스키마 + Edge Function |
| 3.5 | C | 감사 로그 | 주문 상태 변경 이력 |
| 3.6 | C | 분석용 RPC 함수 추가 | 병원별/제품별 통계 |
| 3.7 | D | Vercel Cron Jobs | 일일/월별 리포트 자동 생성 |

### Phase 4: 사용자 경험 강화 (8주 ~ 12주)
...
| 4.3 | B | 다크 모드 | 모바일과 일관된 테마 |
| 4.4 | A | AI 모델 업그레이드 | 최신 On-device AI 모델 |
| 4.5 | D | Vercel Analytics | 성능 모니터링 + Web Vitals |
| 4.6 | D | 커스텀 도메인 | 프로덕션 도메인 연결 |
