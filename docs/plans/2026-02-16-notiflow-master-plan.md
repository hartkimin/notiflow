...
| # | 영역 | 과제 | 산출물 | 상태 |
|---|------|------|--------|------|
| 5.1 | B | PWA 지원 | `manifest.json` + SW 캐시 + 설치 가능 | ✅ |
| 5.2 | B | 다국어 (i18n) | 한국어/영어 | ⬜ (향후) |
| 5.3 | A | 배송 추적 연동 | 택배사 API 연동 | ⬜ (외부 API 필요) |
| 5.4 | C | 멀티테넌트 | 복수 업체 지원 구조 | ⬜ (아키텍처 결정 필요) |
| 5.5 | C | 데이터 보존 정책 | `00009_data_retention.sql` + `/api/cron/archive` | ✅ |
| 5.6 | A | iOS 버전 검토 | KMP 또는 Swift 평가 | ⬜ (향후) |
